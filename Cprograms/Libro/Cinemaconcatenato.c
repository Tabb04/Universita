...
int main(void){
    ...
    do{
        ...
        printf("4. Trasferisci dati su file\n"); 
        printf("0. Esci\n");
...
void ScriviSuFile(Nodo* lista){
    ...
    nome_utente[strcspn(nome_utente, "\n")] = '\0';
    snprintf(nome_completo, sizeof(nome_completo), "%s.txt", nome_utente);

    file = fopen(nome_completo, "w");
    
    Nodo* current = lista;
    while(current != NULL){
        fprintf(file, "Nome: %s, Posto: %d\n", current->nome, current->posto);
        current = current->next;
    }
    fclose(file);
    printf("Prenotazioni salvate nel file %s\n", nome_completo);
    puts("------------------------------------------");
    return;
}