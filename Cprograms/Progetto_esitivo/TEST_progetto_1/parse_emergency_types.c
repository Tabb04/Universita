#define _POSIX_C_SOURCE 200809L //per warnings
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
...
bool parse_emergency_types(const char* nome_file, system_config_t* config){
    ...
    while((fgets(riga, sizeof(riga), file)) && (indice_emergenza < type_count)){
        ...
        char buff_prio[10];
        if((riga_fine_prior == riga_ptr) || ((size_t)(riga_fine_prior - riga_ptr) > sizeof(buff_prio))){    //size_t perchè compilatore si lamentava
