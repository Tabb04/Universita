...
bool invia_emergenze_func(mqd_t desc_coda, const char *emergency_name, int x, int y, int delay_secs) {
...
int main(int argc, char *argv[]) {
    ...
    mqd_t desc_coda = mq_open(mq_name, O_WRONLY);
    ...
    if (strcmp(argv[1], "-f") == 0) {
        ...
        while (fgets(line_buffer, sizeof(line_buffer), file)) {
            ...
            if (sscanf(line_buffer, "%s %d %d %d", name_from_file, &x_from_file, &y_from_file, &delay_from_file) == 4) {
                if (!invia_emergenze_func(desc_coda, name_from_file, x_from_file, y_from_file, delay_from_file)) {
        ...
        if (!invia_emergenze_func(desc_coda, emergency_name_arg, x_arg, y_arg, delay_arg)) {
            success = false;
        }
    }
