...
struct timespec ts = { .tv_sec = 1, .tv_nsec = 0 };			    // 1 secondo, 0 nanosecondi
...
//la spinlock fa un while che continua finchè non ho per me la lock
//cioè da true finche non ho la lock per me
//una volta che acquisisco la lock, divento falso e mi tengo la lock per me


void spinunlock(){
    atomic_flag_clear(&lock);
}
//la unlock sblocca la lock
