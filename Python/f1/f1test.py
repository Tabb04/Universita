...
if mode == "singolo":
    ...
    for label in stats_labels:
        ...
        print(f"{label.capitalize():<12} | {n1}: {v1} ({pct1:.1f}%) | {n2}: {v2} ({pct2:.1f}%) | SENZA DNF -> {n1}: {v1_nd} ({pct1_nd:.1f}%) | {n2}: {v2_nd} ({pct2_nd:.1f}%)")
    ...
    if not common_years:
        ...
        for idx, (driver_id, name, years) in enumerate([(d1_id, n1, common_years), (d2_id, n2, common_years)]):
            ...
            print(f"Punti: {tot.get('punti', 0)}")

            # Per stagione con DNF
            print("\n— Per stagione (con DNF) —")
            for year in years:
                s = stats_with_dnf["per_anno"][year]
                print(f"{year}: {s['gare']} gare | Vittorie: {s['vittorie']} ({safe_pct(s['vittorie'], s['gare']):.1f}%) | "
        ...
        # 📅 Confronto stagione per stagione (anni comuni) — con percentuali
        ...
        for year in common_years:
            ...
            print(f"{year} | {n1.split()[-1]} -> Vitt:{a['vittorie']} ({safe_pct(a['vittorie'], a['gare']):.1f}%) Podi:{a['podi']} ({safe_pct(a['podi'], a['gare']):.1f}%) DNF:{a['dnf']} ({safe_pct(a['dnf'], a['gare']):.1f}%) Pole:{a['pole']} ({safe_pct(a['pole'], a['gare']):.1f}%) GV:{a['giri_veloci']} ({safe_pct(a['giri_veloci'], a['gare']):.1f}%) | "
                  f"{n2.split()[-1]} -> Vitt:{b['vittorie']} ({safe_pct(b['vittorie'], b['gare']):.1f}%) Podi:{b['podi']} ({safe_pct(b['podi'], b['gare']):.1f}%) DNF:{b['dnf']} ({safe_pct(b['dnf'], b['gare']):.1f}%) Pole:{b['pole']} ({safe_pct(b['pole'], b['gare']):.1f}%) GV:{b['giri_veloci']} ({safe_pct(b['giri_veloci'], b['gare']):.1f}%)")

        # 📅 Confronto stagione per stagione (anni comuni) SENZA DNF — con percentuali
        ...
        for year in common_years:
            ...
            print(f"{year} | {n1.split()[-1]} -> Vitt:{a['vittorie']} ({safe_pct(a['vittorie'], a['gare']):.1f}%) Podi:{a['podi']} ({safe_pct(a['podi'], a['gare']):.1f}%) Pole:{a['pole']} ({safe_pct(a['pole'], a['gare']):.1f}%) GV:{a['giri_veloci']} ({safe_pct(a['giri_veloci'], a['gare']):.1f}%) | "
                  f"{n2.split()[-1]} -> Vitt:{b['vittorie']} ({safe_pct(b['vittorie'], b['gare']):.1f}%) Podi:{b['podi']} ({safe_pct(b['podi'], b['gare']):.1f}%) Pole:{b['pole']} ({safe_pct(b['pole'], b['gare']):.1f}%) GV:{b['giri_veloci']} ({safe_pct(b['giri_veloci'], b['gare']):.1f}%)")
