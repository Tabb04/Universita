 ...
 *
 * gcc pcount_capabilities.c -o pcount -lpcap -lcap
 *
*/
...
int drop_privileges_capabilities(void) {
  ...
  cap_value_t cap_list[] = {
    CAP_NET_RAW,      /* Required for packet capture */
    CAP_NET_ADMIN,    /* Required for setting promiscuous mode */
    CAP_DAC_OVERRIDE, /* Required for reading pcap files if needed */
    CAP_DAC_READ_SEARCH /* Required for reading directories if needed */
  };
  int num_caps = sizeof(cap_list) / sizeof(cap_list[0]);

  /* Ensure we keep capabilities after setuid() if we were to call it */
  ...
  /* Set only the capabilities we need */
  ...
  /* Optional: drop to a non-root user while keeping capabilities */
...
int main(int argc, char* argv[]) {
  ...
  if(geteuid() != 0) {
    printf("Please run this tool as superuser (or with CAP_NET_RAW and CAP_NET_ADMIN capabilities)\n");
    return(-1);
  }
  ...
  /* Replace old drop_privileges with capabilities-based approach */
